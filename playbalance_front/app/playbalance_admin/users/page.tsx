                  ...
                  <td className="py-3 px-2">
                    {editingId === u.id ? (
                      <Input 
                        ...
                        placeholder="admin, playbalance_owner"
                      />
                    ) : (
                      <div className="flex flex-wrap gap-1 max-w-[150px]">
                        {u.roles.map((r: string) => (
                          <span key={r} className={`px-2 py-0.5 rounded-full text-xs font-bold bg-zinc-800 border border-zinc-700 ${r === 'admin' ? 'text-yellow-500' : r === 'playbalance_owner' ? 'text-red-500' : 'text-zinc-300'}`}>
                            {r}
                          </span>
                        ))}
                      </div>
