...
export const playbalanceAdminNavItems = [
  {
    name: "Visão Geral",
    href: "/playbalance_admin",
    icon: Activity,
  },
  {
    name: "Usuários",
    href: "/playbalance_admin/users",
    icon: Users,
  },
  {
    name: "Conteúdo & Uso",
    href: "/playbalance_admin/content",
    icon: Database,
  },
  {
    name: "Técnico",
    href: "/playbalance_admin/technical",
    icon: Server,
  },
  {
    name: "Transações",
    href: "/playbalance_admin/transactions",
    icon: CreditCard,
  },
  {
    name: "Acessos do App",
    href: "/playbalance_admin/logs",
    icon: ShieldCheck,
  },
]

export default function PlayBalanceAdminLayout({
  children,
}: {
  ...
  useEffect(() => {
    async function verify() {
      try {
        ...
        if (!u?.role?.includes("playbalance_owner")) {
          router.push("/")
        } else {
          setUser(u)
        }
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="flex-1 overflow-y-auto space-y-2">
        {playbalanceAdminNavItems.map((item) => {
          const isActive = item.href === '/playbalance_admin'
            ? pathname === item.href
