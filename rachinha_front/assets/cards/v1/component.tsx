...
export function SvgCardV1Base({ colorSet, idPrefix }: SvgCardV1Props) {
...
interface SvgCardV1BorderProps extends SvgCardV1Props {
  strokeWidth: number
  showOuterBorder: boolean
}

export function SvgCardV1Border({ colorSet, idPrefix, strokeWidth, showOuterBorder }: SvgCardV1BorderProps) {
  const goldBorderId = `${idPrefix}-gold-border`
  const borderMaskId = `${idPrefix}-border-mask`
  ...
  return (
    <svg viewBox={USER_PROFILE_CARD_DESIGN.viewBox} xmlns="http://www.w3.org/2000/svg" className="absolute inset-0 h-full w-full pointer-events-none" style={{ zIndex: 3 }}>
      ...
      {/* Branding: logo icon + RACHINHA.COM — posicionado no pill superior */}
