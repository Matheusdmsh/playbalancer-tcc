...
class BookingService:
    ...
    async def create_recurring_booking(self, user_id: str, data: dict):
        ...
        for booking_id in ids:
            ...
            for member_id in invited_members:
                if member_id not in booking_obj.get('players', []):
                    invite_data = {
    ...
    async def add_player_to_booking(self, user_id: str, booking_id: str, player_id: str, skill_level: float) -> dict:
        ...
        if len(booking.get('players', [])) < booking.get('max_players', 0) or booking.get('max_players', 0) == 0:
            ...
            await self._sync_organized_teams_with_players(booking_id)
    ...
    @staticmethod
    def _booking_player_id(player: Any) -> Optional[str]:
        if isinstance(player, dict):
            player = player.get('id') or player.get('_id')
        return str(player) if player is not None else None

    async def _sync_organized_teams_with_players(self, booking_id: str) -> None:
        """Keeps an existing draw aligned with the confirmed-player list."""
        booking = await self.booking_repo.get_by_id(booking_id)
        if not booking:
            return

        organized = booking.get('organized_teams')
        raw_teams = organized.get('teams') if isinstance(organized, dict) else None
        ...
        for player in booking.get('players', []):
            player_id = self._booking_player_id(player)
            if player_id and player_id not in confirmed_players:
                confirmed_players[player_id] = player
                confirmed_order.append(player_id)
        ...
        for raw_team in raw_teams:
            if not isinstance(raw_team, list):
                continue
            team = []
            for player_id in raw_team:
                normalized_id = self._booking_player_id(player_id)
                if normalized_id and normalized_id in confirmed_players and normalized_id not in assigned_ids:
                    team.append(normalized_id)
        ...
        history = booking.get('organized_teams_history', []) or []
        latest_draw = history[-1] if history and isinstance(history[-1], dict) else {}
        players_per_team = latest_draw.get('players_per_team') or max((len(team) for team in teams), default=0)
        if players_per_team == 0:
            return

        waiting_ids = []
        for player_id in organized.get('reserves', []) or []:
            normalized_id = self._booking_player_id(player_id)
            if normalized_id and normalized_id in confirmed_players and normalized_id not in assigned_ids and normalized_id not in waiting_ids:
                waiting_ids.append(normalized_id)
        for player_id in confirmed_order:
            if player_id not in assigned_ids and player_id not in waiting_ids:
                waiting_ids.append(player_id)

        for team in teams:
            while len(team) < players_per_team and waiting_ids:
                promoted_id = waiting_ids.pop(0)
                team.append(promoted_id)
                assigned_ids.add(promoted_id)

        team_skills = []
        for team in teams:
            total_skill = 0.0
            for player_id in team:
                player = confirmed_players[player_id]
                skill = player.get('skill_level', 0) if isinstance(player, dict) else 0
                total_skill += self._round_skill_for_draw(skill)
    ...
    async def accept_invite(self, user_id: str, invite_id: str) -> dict:
        ...
        if len(booking.get('players', [])) < booking.get('max_players', 0) or booking.get('max_players', 0) == 0:
            ...
            await self._sync_organized_teams_with_players(booking['_id'])
            await self.invite_repo.update_invite_status(invite_id, 'accepted')
    ...
    async def sync_group_member_to_bookings(self, group_id: str, member_id: str, action: Literal['add', 'remove']):
        ...
        for booking in bookings_associated:
            ...
            if action == 'add':
                if member_id not in current_player_ids:
